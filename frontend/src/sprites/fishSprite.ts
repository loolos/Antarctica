...
export interface HalfEatenFishSpriteOptions {
  x: number;
  y: number;
  age?: number;
...
/**
 * Draw fish remains on ice floe (half-eaten icon).
 */
export function drawHalfEatenFish(
  ctx: CanvasRenderingContext2D,
  options: HalfEatenFishSpriteOptions
): void {
  const { x, y, age = 0, animationTime = 0 } = options;
  const fade = Math.max(0.35, 1 - age * 0.02);
  const bob = Math.sin(animationTime * Math.PI * 2) * 0.6;

  ctx.save();
  ctx.translate(x, y + bob);
  ctx.globalAlpha = fade;

  // Remaining back half of fish body.
  ctx.fillStyle = '#4f97df';
  ctx.beginPath();
  ctx.ellipse(2, 0, 5.5, 3, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  // Exposed bite edge + tiny bones to emphasize "half eaten".
  ctx.fillStyle = '#f4f7fb';
  ctx.fillRect(-3.4, -3, 1.8, 6);

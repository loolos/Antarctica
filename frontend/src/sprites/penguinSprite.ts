...
export function drawPenguin(ctx: CanvasRenderingContext2D, options: SpriteOptions): void {
  ...
  // Left wing
  ctx.beginPath();
  ctx.ellipse(-8 * scale, 0, 3 * scale, 8 * scale, -0.3, 0, Math.PI * 2);
  ctx.fill();
  // Right wing (back)
  ctx.beginPath();
  ctx.ellipse(8 * scale, 2 * scale, 3 * scale, 6 * scale, 0.3, 0, Math.PI * 2);
  ctx.fill();
  ...
  // Energy indicator (small glow or outline)
  if (!isHealthy) {
